...
async function register(req: any) {
    const [email]: any = await collections.users?.find({email: req.body.email}).toArray();
    if(email) return "Podany adres email został już zarejestrowany"
    
    const [login]: any = await collections.users?.find({login: req.body.login}).toArray();
...
async function confirm(req: any) {
    ...
    if(user) {
        await collections.users?.updateOne({ confirmEmailToken }, { $unset: { confirmEmailToken: 1 } })
